...
async function cargarPreguntas() {
    ...
    try {
        ...
        // Limpieza absoluta del contenedor
        contenedor.innerHTML = ""; 

        preguntas.forEach((p, index) => {
            const div = document.createElement('div');
            div.className = "mb-8 p-6 bg-white rounded-xl border border-slate-200 shadow-sm animate-fade-in";
            div.innerHTML = `
        ...
        btn.className = "w-full bg-blue-900 text-white py-4 rounded-xl font-bold mt-6 shadow-lg hover:bg-blue-800 transition-all transform active:scale-95";
        btn.onclick = calcularResultados;
        contenedor.appendChild(btn);

    } catch (error) {
        contenedor.innerHTML = "<p class='text-red-500 p-4 bg-red-50 rounded'>Error crítico: No se pudieron cargar las preguntas desde el JSON.</p>";
